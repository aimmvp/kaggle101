...
# n = 5270 # number of rows in the file
# SAMPLE_ROW = 310 # desired sample size
csv_file = "input/category_names.csv"
# skip = sorted(random.sample(range(n), n-SAMPLE_ROW))
...
for category_id in category_ids:
    ...
    if item_cnt > 200 and item_cnt < 300 and cate_cnt < 101:
        collection = db['train_' + s_collection_id]
        # collection.drop()
        items = train_collection.find({'category_id': i_collection_id}).limit(150)
